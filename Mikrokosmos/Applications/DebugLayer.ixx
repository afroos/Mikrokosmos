...
#include <Mikrokosmos/Graphics/OpenGL/ImGuiOpenGLRenderer.h>
...
namespace mk
{
	...
	{
		auto& io = ImGui::GetIO();
		io.MouseDown[ToPlatformButton(event.Button())] = true;
	}

	void DebugLayer::OnMouseButtonReleasedEvent(MouseButtonReleasedEvent& event)
	{
		auto& io = ImGui::GetIO();
		io.MouseDown[ToPlatformButton(event.Button())] = false;
	}
	...
	{
		auto& io = ImGui::GetIO();
		io.KeysDown[ToPlatformKey(event.Key())] = true;
	...
	{
		auto& io = ImGui::GetIO();
		io.KeysDown[ToPlatformKey(event.Key())] = false;
	}
